...
週2〜3回・各回最大15件に厳選して通知することで、情報過多から解放され、質の高い技術情報にのみ集中できる世界を実現する。

### 目的
- 技術情報の判断コストをゼロにする（「読むべきか？」の判断をシステムが代行）
- ノイズを徹底的に排除する（無関係な情報・重複・低品質記事を通知しない）
- 最小出力を維持する（各回最大15件、0件でも通知）
...
**受け入れ条件**:
- [ ] SocialProof 4指標統合スコアを算出できる（yamadashy, Hatena, Zenn, Qiita）
- [ ] InterestProfileとの5段階マッチングでInterestスコアを算出できる
- [ ] authority_levelに応じたAuthorityスコアを算出できる
- [ ] 上記3要素の重み付け合算でBuzzスコアを算出できる

**優先度**: P0(必須)

**技術仕様**:
- BuzzScore計算式: `total_score = (social_proof × 0.55) + (interest × 0.35) + (authority × 0.10)`
- 3要素: SocialProof 55%, Interest 35%, Authority 10%
- スコアが高いほど優先度高
...
- [ ] 判定結果は固定フォーマット（JSON）で取得できる: `{"interest_label": "ACT_NOW | THINK | FYI | IGNORE", "confidence": 0.0~1.0, "summary": "記事の内容を簡潔に要約", "tags": ["Kotlin", "Claude"]}`
- [ ] 判定プロンプトには以下を含める:
  - 関心プロファイル（強い関心・低優先度のリスト）
  - 記事情報（タイトル、概要、ソース名、公開日時）
  - 判定基準（ACT_NOW: 今すぐ読むべき / THINK: 設計判断に役立つ / FYI: 知っておくとよい / IGNORE: 関心外）
- [ ] JSON形式崩れの場合、再試行（最大BEDROCK_MAX_RETRIES回、デフォルト4回）→ 失敗時はIGNORE扱い
- [ ] Throttling/ServiceUnavailableの場合、指数バックオフ+ジッター付きで同様に再試行
...
**ユーザーストーリー**:
ユーザーとして、週2〜3回・各回最大15件に厳選された記事のみを通知されたい

**受け入れ条件**:
- [ ] Interest Label順に優先順位付けできる（ACT_NOW > THINK > FYI > IGNORE）
- [ ] Composite Score（InterestLabel + 外部話題性の重み付き混合）でソートできる
- [ ] 最大15件を選定できる
- [ ] 同一ドメイン偏り制御ができる（FINAL_SELECT_MAX_PER_DOMAIN設定値に従う、デフォルト0=制限なし）
...
**受け入れ条件**:
- [ ] メール本文に記事一覧（タイトル、URL、Interestラベル、要約、タグ）を含める
...
1. [タイトル]
   URL: https://...
   ラベル: ACT_NOW
   タグ: PostgreSQL, インデックス
   要約: PostgreSQLのインデックス戦略について実践的な知見
...
### アーキテクチャ制約
- **技術スタック**: AWS Lambda（Python 3.14）、DynamoDB、Bedrock、SES、EventBridge
